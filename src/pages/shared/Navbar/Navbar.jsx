...
// 🟢 1. Dynamic Environment Base URL Setup (Prevents deployment network dropouts)
...
const Navbar = () => {
  ...
  // 📡 2. Updated to fetch admin status using the dynamic API environment variable
  useEffect(() => {
    if (user?.email) {
      fetch(`${API_BASE_URL}/api/users/admin/${user.email}`)
        .then((res) => {
          if (!res.ok) throw new Error('Network validation response down');
          return res.json();
        })
        .then((data) => {
          setIsAdmin(data.admin);
        })
        .catch((err) => console.error("❌ Error verifying admin status endpoint:", err));
    } else {
      setIsAdmin(false);
    }
  }, [user?.email]);
  ...
  return (
    <header className="w-full bg-white border-b border-gray-100 sticky top-0 z-50 px-4 md:px-6 py-3">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        ...
        {/* AUTHENTICATION & SHOPPING CART CONTROL ACTION BLOCK */}
